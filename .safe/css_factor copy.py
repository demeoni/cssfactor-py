...
def tokenize(css: str) -> List[Tuple[str, str]]:
    ...
    for mo in re.finditer(tok_regex, css):
        kind = mo.lastgroup
        value = mo.group()
...
class Parser:
    ...
    def check(self, type):
        if self.is_at_end():
            return False
        return self.tokens[self.current][0] == type
    ...
    def consume(self, *types):
        if self.check(types[0]):
            return self.advance()
        raise Exception(f"Expected {types}, found {self.tokens[self.current][0]}")

    def previous(self):
        return self.tokens[self.current - 1]

def factor_css(stylesheet: Stylesheet) -> Stylesheet:
    factored_statements = []
...
def explode_css(stylesheet: Stylesheet) -> Stylesheet:
    exploded_statements = []
    for statement in stylesheet.statements:
        if isinstance(statement, Ruleset):
